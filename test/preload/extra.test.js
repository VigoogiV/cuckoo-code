...
test('randomDelay 返回 2000-3999ms', async () => {
  const { randomDelay } = await import('../../src/preload/dom/chat-input.js');
  for (let i = 0; i < 10; i++) {
    const d = randomDelay();
...
test('updateProjectDirDisplay 更新显示', async () => {
  ...
  const { updateProjectDirDisplay } = await import('../../src/preload/overlay/project-dir.js');
...
test('renderSessions API 不可用显示提示', async () => {
  ...
  global.window = {};
  const { renderSessions } = await import('../../src/preload/dom/session-list.js');
